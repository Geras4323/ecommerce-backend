...
func Auth(g *echo.Group) {
	...
	g.POST("/recovery", controllers.StartPasswordRecovery)
	g.POST("/recover-password", controllers.RecoverPassword)

	g.PATCH("/change-password", controllers.ChangePassword, auth.WithAuth)
}
